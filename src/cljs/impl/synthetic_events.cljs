...
(defn mouse-in-event []
  (events/pattern :mouse-in
                  ...
                  (fn [e s]
                    (events/schedule s #(events/clear-state s) :start)
                    (when-let [previous-component (is-component (events/get-context s :in-out))]
                      (events/schedule s (fn [] (events/trigger-bus-event e (merge (events/enrich previous-component) {:type "mouse-out"}))) :completed))
                  ...
                  (fn [e s]
                    ...
                    (let [previous-component (is-component (events/get-context s :in-out))]
                      (if-not (nil? (:component e))
                        (do
                          (events/set-context s :in-out (:component e))
                          (events/schedule s (fn [] (events/trigger-bus-event e {:type "mouse-in"})) :completed))
